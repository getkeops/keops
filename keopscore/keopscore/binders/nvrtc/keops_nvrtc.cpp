...
std::cout << "allocating offsets_d" << std::endl;
...
std::cout << "allocating slices_x_d" << std::endl;
    CUDA_SAFE_CALL(
...
std::cout << "allocating ranges_y_d" << std::endl;
    CUDA_SAFE_CALL(
  ...
  // -----------------------------------------------------
std::cout << "allocating lookup_d" << std::endl;
  CUDA_SAFE_CALL(
...
public:
  ...
  KeOps_module(int device_id, int nargs_, const char *target_file_name) {
    ...
    std::cout << "in KeOps_module init" << std::endl;
...
std::cout << "allocating buffer" << std::endl;
    ...
    std::cout << "finished KeOps_module init" << std::endl;
  }

  ~KeOps_module() {
    std::cout << "in KeOps_module end" << std::endl;
    SetContext();
    ...
    delete[] target;
    std::cout << "finished KeOps_module end" << std::endl;
  }
...
std::cout << "allocating p_data_outB" << std::endl;
      CUDA_SAFE_CALL(
...
std::cout << "freeing p_data_outB" << std::endl;
    ...
    if (tagHostDevice == 0) {
      std::cout << "freeing p_data" << std::endl;
      CUDA_SAFE_CALL(cuMemFree(p_data));
    }

    if (RR.tagRanges == 1) {
      std::cout << "freeing lookup_d" << std::endl;
      CUDA_SAFE_CALL(cuMemFree((CUdeviceptr)lookup_d));
      if (SS.nbatchdims > 0) {
        std::cout << "freeing slices_x_d" << std::endl;
        CUDA_SAFE_CALL(cuMemFree((CUdeviceptr)slices_x_d));
        std::cout << "freeing ranges_y_d" << std::endl;
        CUDA_SAFE_CALL(cuMemFree((CUdeviceptr)ranges_y_d));
        std::cout << "freeing offsets_d" << std::endl;
