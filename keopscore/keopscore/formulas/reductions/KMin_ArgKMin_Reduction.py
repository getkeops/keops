...
class KMin_ArgKMin_Reduction(Reduction):
    ...
    def ReducePair(self, acc, xi):
        ...
        p = c_variable("size_t", new_c_varname("p"))
        q = c_variable("size_t", new_c_varname("q"))
    ...
    def ReducePairShort(self, acc, xi, ind):
        ...
        l = c_variable("size_t", new_c_varname("l"))
        k = c_variable("size_t", new_c_varname("k"))
        tmpl = c_variable(dtype, new_c_varname("tmpl"))
        indtmpl = c_variable("size_t", new_c_varname("indtmpl"))
        return f"""
                    {{
                        {xik.declare()}
                        {l.declare()}
                        {use_pragma_unroll()}
                        for(size_t {k.id}=0; {k.id}<{fdim}; {k.id}++) {{
                            {xik.assign(xi[k])}
                            {use_pragma_unroll()}                 
