...
class Var(Operation):
    ...
    def __init__(self, ind=None, dim=None, cat=None, label=None, params=None):
        # N.B. init via params keyword is used for compatibility with base class.
        if ind is None:
            # here we assume dim and cat are also None, and
            # that params is a tuple containing ind, dim, cat, label
            ind, dim, cat, label = params
        super().__init__(params=(ind, dim, cat, label))
        if label is None:
