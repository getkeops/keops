          ...
          if (nbatchdims == 0) {
            for (signed long int jrel = 0;
...

              {
                ...
                // Var(0,6,2)[(3*Var(1,1,0)+Var(2,1,1))].

                double out_add_0[1];

                {
                  // Starting code block for 3*Var(1,1,0)+Var(2,1,1).

                  double out_mult_0[1];

                  {
                    // Starting code block for 3*Var(1,1,0).

                    double out_intcst_0[1];

                    {
                      // Starting code block for 3.

                      *out_intcst_0 = (double)((float)3);

                      // Finished code block for 3.
                    }
                    ...
                    // Finished code block for 3*Var(1,1,0).
                  }
                  ...
                  // Finished code block for 3*Var(1,1,0)+Var(2,1,1).
                }

                (*fout) = ((arg_0[0] + 0 * 6)[(signed long int)(*out_add_0)]);
  ...
  if (nbatchdims == 0) {
    {
      ...
      for (signed long int v = 0; v < 6; v++) {
        param_loc[a] = arg_0[0][0 * 6 + v];
        a++;
      }
    }
  ...
  } else {
    {
      ...
      for (signed long int v = 0; v < 6; v++) {
        param_loc[a] = arg_0[0][(0 + indices_p[0]) * 6 + v];
        a++;
      }
    }
  }
  ...
  if (i < end_x) {
    ...
    if (nbatchdims == 0) {
    ...
    } else {
      {
        ...
        for (signed long int v = 0; v < 1; v++) {
          xi[a] = arg_0[1][(threadIdx.x + indices_i[0]) * 1 + v];
          a++;
        }
      }
      // Possibly, with offsets as we support broadcasting over batch dimensions
