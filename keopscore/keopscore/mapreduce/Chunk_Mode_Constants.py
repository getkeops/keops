...
class Chunk_Mode_Constants:
    def __init__(self, red_formula):
        ...
        chunked_formula = formula.chunked_formulas(chunks.get_dimchunk())[0]
        self.dim_org = chunked_formula["dim_org"]
        self.nchunks = 1 + (self.dim_org - 1) // chunks.get_dimchunk()
        self.dimlastchunk = self.dim_org - (self.nchunks - 1) * chunks.get_dimchunk()
