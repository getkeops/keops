...
def do_chunk_sub_ranges(
    ...
    return f"""
                ...
                if ({i.id} < {end_x.id}) {{ // we compute only if needed
            		...
            		for (size_t jrel = 0; (jrel < blockDim.x) && (jrel < {end_y.id} - jstart); jrel++, yjrel += {chk.dimy}) {{
...
class GpuReduc1D_ranges_chunks(MapReduce, Gpu_link_compile):
    ...
    def get_code(self):
        ...
        last_chunk = c_variable("size_t", f"{chk.nchunks-1}")
                          ...
                          for( size_t index = start_slice ; index < end_slice ; index++ ) {{
                              if( (index+1 >= end_slice) || (ranges_y[2*index+2] != ranges_y[2*index+1]) ) {{
                                  ...
                                  for(size_t jstart = start_y, tile = 0; jstart < end_y; jstart += blockDim.x, tile++) {{
                                      ...
                                      for (size_t jrel = 0; (jrel < blockDim.x) && (jrel < end_y - jstart); jrel++) {{
                                  ...
                                  // looping on chunks (except the last)
                          		  {use_pragma_unroll()}
                          		  for (size_t chunk=0; chunk<{chk.nchunks}-1; chunk++) {{
                                      {chunk_sub_routine}
                                  }}
                                  // last chunk
                                  ...
                                  if (i < end_x) {{
                                      ...
                                      if (nbatchdims == 0) {{
                                          for (size_t jrel = 0; (jrel < blockDim.x) && (jrel <end_y - jstart); jrel++, yjrel += {chk.dimy}) {{
