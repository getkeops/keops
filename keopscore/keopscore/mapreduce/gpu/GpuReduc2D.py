...
class GpuReduc2D(MapReduce, Gpu_link_compile):
    ...
    def get_code(self):
        ...
        jrelloc = c_variable("size_t", "(blockDim.x*blockIdx.y+jrel)")

        tid = c_variable("size_t", "tid")
        ...
        self.code = f"""
                        ...
                        extern "C" __global__ void GpuConv2DOnDevice(size_t nx, size_t ny, {dtype} *out, {dtype} **{arg.id}) {{
                            ...
                            size_t j = blockIdx.y * blockDim.x + threadIdx.x; // Same blockDim in x and y : squared tiles.
                            ...
                            if(i<nx) {{ // we compute x1i only if needed
                                ...
                                for(size_t jrel = 0; (jrel<blockDim.x) && ((blockDim.x*blockIdx.y+jrel)< ny); jrel++, yjrel+={dimy}) {{
