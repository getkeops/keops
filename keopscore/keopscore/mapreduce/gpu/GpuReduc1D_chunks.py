...
def do_chunk_sub(
    ...
    return f"""
                ...
                if ({i.id} < {nx.id}) {{ // we compute only if needed
                    ...
                    for (size_t jrel = 0; (jrel < blockDim.x) && (jrel < {ny.id} - jstart); jrel++, yjrel += {chk.dimy}) {{
    ...
    def get_code(self):
        ...
        self.code = f"""
                        ...
                        extern "C" __global__ void GpuConv1DOnDevice(size_t nx, size_t ny, {dtype} *out, {dtype} **{arg.id}) {{
                          ...
                          for (size_t jstart = 0, tile = 0; jstart < ny; jstart += blockDim.x, tile++) {{
                            ...
                            if (i < nx) {{ // we compute x1i only if needed
                              for (size_t jrel = 0; (jrel < blockDim.x) && (jrel < ny - jstart); jrel++) {{
                            ...
                            if (i < nx) {{
                                ...
                                for (size_t jrel = 0; (jrel < blockDim.x) && (jrel < ny - jstart); jrel++, yjrel += {chk.dimy}) {{
