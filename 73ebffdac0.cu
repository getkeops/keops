...
extern "C" __global__ void GpuConv1DOnDevice(signed long int nx, signed long int ny, float* out, float** arg2)
{
    ...
    {
        #pragma unroll(64)
        for(int k2 = 0; k2<3; k2++)    acc[k2] = 0.0f;
        signed long int a = 0;
        {
    ...
    {
        #pragma unroll(64)
        for(int k7 = 0; k7<3; k7++)    (out+(i*3))[k7] = acc[k7];
    }
}