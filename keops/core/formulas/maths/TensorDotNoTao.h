...
namespace keops {
    ...
    namespace loop_impl {
        ...
        template < size_t... Is >
        struct loop_t_arr< std::index_sequence< Is...>> {
    ...
    static constexpr auto make_array(std::index_sequence<I...>) {
    ...
    template<std::size_t N>
    static constexpr auto reverse_array(std::array<size_t, N> arr) {
        std::array<size_t, N> res{};
    ...
    template<std::size_t N, size_t M>
    static constexpr auto map_array(std::array<size_t, N> ind, std::array<size_t, M> arr1) {
        std::array<size_t, N> res{};
        for (size_t i= 0; i < N; i++){
            res[i] = arr1[ind[i]];
        }
    ...
    template<std::size_t N>
    static constexpr auto permutate_array(std::array<size_t, N> perm, std::array<size_t, N> arr) {
        std::array<size_t, N> res{};
        for (size_t i= 0; i < N; i++){
            res[perm[i]] = arr[i];
        }
    ...
    template<std::size_t N, size_t M>
    constexpr auto first_array(const std::array<size_t, M> arr) {
        std::array<size_t, N> res;
        for (size_t i= 0; i < N; i++){
            res[i] = arr[i];
        }
...
//    static_assert(permutate_array(permute_arr, permute_arr) == make_index_array< keepdim_size >(), "In TensorDotNoTao: PERMUTE should be a permutation index_sequence.");  //TODO
        ...
        static constexpr std::array<size_t, DIMFA::size() > ala =
                concat_array(make_index_array< keepdim_a.size() >(), make_range_array< keepdim.size(), number_of_dimloop>());
        ...
        static constexpr std::array<size_t, DIMFB::size() > bla =
                concat_array(make_index_array< keepdim_a.size(), keepdim.size() >, make_index_array< keepdim.size(), number_of_dimloop>());
        static constexpr std::array<size_t, DIMFB::size() > bli = concat_array(indices_keepdim_b, contfb_arr);
        static constexpr std::array<size_t, DIMFB::size() > list_indices_b_intot = permutate_arra(bli, bla);
        ...
        static constexpr std::integer_sequence<size_t> DIMFA_GRAD_imp = make_seq<dimfa_grad>();
        using DIMFA_GRAD = typeof(DIMFA_GRAD_imp);

        static constexpr std::array< size_t, DIMFB::size() - CONTFB::size() > contfa_grad=map_array(list_indices_keepdim_b_inout, permute_arr);
        static constexpr std::integer_sequence<size_t> CONTFA_GRAD_imp = make_seq<contfa_grad>();
        using CONTFA_GRAD = typeof(CONTFA_GRAD_imp);

        static constexpr std::array< size_t, DIMFA::size() - CONTFA::size() > contfb_grad=map_array(list_indices_keepdim_a_inout, permute_arr);
        static constexpr std::integer_sequence<size_t> CONTFB_GRAD_imp = make_seq<contfb_grad>();
        using CONTFB_GRAD = typeof(CONTFB_GRAD_imp);

        static constexpr std::integer_sequence<size_t> loopdim_imp = make_seq<loopdim>();
        using loopdim_t = typeof(loopdim_imp);
        ...
        DEVICE constexpr static tensordot_indices_arr compute_tensordot_indices_arr(IND) {
            ...
            // a_indices
            std::array<size_t, DIMFA::size() > list_indices_a = map_array(list_indices_a_intot, ind_arr);
#if C_CONTIGUOUS
            ...
            // b_indices
            std::array<size_t, DIMFB::size() >  list_indices_b  = map_array(list_indices_b_intot, ind_arr);
#if C_CONTIGUOUS
...
/*
  ...
  >;*/
        template<class V, class GRADIN>
        using DiffT = typename A::template DiffT<V, Mult<Exp<A>, GRADIN>>;
