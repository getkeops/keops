...
struct pack< N, NS... > {
  ...
  static const int
          SUM = N +
                NEXT::SUM;                           // The total "memory footprint" of pack<N,NS...> is computed recursively.
  ...
  HOST_DEVICE static void load(int i, TYPE* xi, TYPE** px) {
    ...
    NEXT::load(i, xi + FIRST,
               px + 1);              // Then,  load the i-th line of px[1:] -> xi[ FIRST : ] (recursively)
  }
  ...
  HOST_DEVICE static void load(int i, TYPE* xi, TYPE** px, int* offsets) {
    ...
    NEXT::load(i,
               ...
               px + 1,
               offsets +
               1);                        // Then,  load the i-th line of px[1:] -> xi[ FIRST : ] (recursively)
  }
