  ...
  HOST_DEVICE static void Eval(int i, int chunk_index, TYPE *xi, TYPE **px, int *offsets) {
    ...
    for (int k = 0; k < DIM_CHUNK_LOAD; k++) {
      xi[k] = px[FIRSTIND][true_i * DIM_ORG + chunk_index*DIM_CHUNK + k];
        printf("k=%d, FIRSTIND=%d, i=%d, true_i=%d, DIM_ORG=%d, chunk_index=%d, DIM_CHUNK=%d, true_i * DIM_ORG + chunk_index*DIM_CHUNK + k=%d, xi[k]=%f\n",k,FIRSTIND,i,true_i,DIM_ORG,chunk_index,DIM_CHUNK,true_i * DIM_ORG + chunk_index*DIM_CHUNK + k);
    }
