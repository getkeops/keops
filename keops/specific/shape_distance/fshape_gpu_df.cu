...
int fshape_gpu_df(double ooSigmax2,double ooSigmaf2, double ooSigmaXi2,
                  TYPE* x_h, TYPE* y_h,
                  TYPE* f_h, TYPE* g_h,
    ...
    if(dimPoint==1 && dimSig==1 && dimVect==1)
        fshape_scp_df_OnDevice<TYPE,1,1,1><<<gridSize,blockSize,blockSize.x*(dimVect+dimSig+dimPoint)*sizeof(TYPE)>>>
        (ooSigmax2,ooSigmaf2,ooSigmaXi2, x_d, y_d, f_d, g_d, alpha_d, beta_d, gamma_d, nx, ny);
    else if(dimPoint==3 && dimSig==1 && dimVect==1)
        fshape_scp_df_OnDevice<TYPE,3,1,1><<<gridSize,blockSize,blockSize.x*(dimVect+dimSig+dimPoint)*sizeof(TYPE)>>>
        (ooSigmax2,ooSigmaf2,ooSigmaXi2, x_d, y_d, f_d, g_d, alpha_d, beta_d, gamma_d, nx, ny);
    else if(dimPoint==2 && dimSig==1 && dimVect==1)
        fshape_scp_df_OnDevice<TYPE,2,1,1><<<gridSize,blockSize,blockSize.x*(dimVect+dimSig+dimPoint)*sizeof(TYPE)>>>
        (ooSigmax2,ooSigmaf2, ooSigmaXi2, x_d, y_d, f_d, g_d, alpha_d, beta_d, gamma_d, nx, ny);
    else if(dimPoint==2 && dimSig==1 && dimVect==2)
        fshape_scp_df_OnDevice<TYPE,2,1,2><<<gridSize,blockSize,blockSize.x*(dimVect+dimSig+dimPoint)*sizeof(TYPE)>>>
        (ooSigmax2,ooSigmaf2, ooSigmaXi2, x_d, y_d, f_d, g_d, alpha_d, beta_d, gamma_d, nx, ny);
    else if(dimPoint==3 && dimSig==1 && dimVect==3)
        fshape_scp_df_OnDevice<TYPE,3,1,3><<<gridSize,blockSize,blockSize.x*(dimVect+dimSig+dimPoint)*sizeof(TYPE)>>>
        (ooSigmax2,ooSigmaf2, ooSigmaXi2, x_d, y_d, f_d, g_d, alpha_d, beta_d, gamma_d, nx, ny);
    else {
        printf("error: dimensions of kernel not implemented in fshape_gpu_df");
        cudaFree(x_d);
        cudaFree(y_d);
        cudaFree(f_d);
        cudaFree(g_d);
...
extern "C" int cudafshape_df(__TYPE__ ooSigmax2,__TYPE__ ooSigmaf2, __TYPE__ ooSigmaXi2, __TYPE__* x_h, __TYPE__* y_h, __TYPE__* f_h, __TYPE__* g_h, __TYPE__* alpha_h, __TYPE__* beta_h, __TYPE__* gamma_h, int dimPoint, int dimSig, int dimVect, int nx, int ny) {
