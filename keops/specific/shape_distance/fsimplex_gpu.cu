    ...
    for(int jstart = 0, tile = 0; jstart < ny; jstart += blockDim.x, tile++) {
        ...
        if(i<nx) { // we compute gammai only if needed
            ...
            for(int jrel = 0; jrel < blockDim.x && jrel<ny-jstart; jrel++, yj+=inc, gj +=inc, betaj+=inc) {
                ...
                gammai += sqrt(norm2Xix * norm2Xiy) * Kernel_geom1(dist2_geom,ooSigmax2) * Kernel_sig1(dist2_sig,ooSigmaf2);
            }
        }
        __syncthreads();
    }
...

{
    ...
    fsimplex<float>(oosigmax2,oosigmaf2,x_f,y_f,f_f,g_f,alpha_f,beta_f,gamma_f,dimpoint,dimsig,dimvect,nx,ny);
