...
// compile with
//		nvcc -std=c++11 -O2 -c test_link_grad.cu
//		./compile "Grad<Scal<Square<Scalprod<X<2,4>,Y<3,4>>>,GaussKernel<P<0>,X<0,3>,Y<1,3>,Y<4,3>>>,X<0,3>,X<5,3>>"
// 		nvcc -o test_link_grad test_link_grad.o "build/Grad<Scal<Square<Scalprod<X<2,4>,Y<3,4>>>,GaussKernel<P<0>,X<0,3>,Y<1,3>,Y<4,3>>>,X<0,3>,X<5,3>>.so"
