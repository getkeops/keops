...
from keops.python_engine import cuda_block_size, dimfinalchunk
...
def do_finalchunk_sub_ranges(dtype, fun_global, varfinal, dimfinalchunk_curr,
    ...
    load_chunks_routine_j_ranges = load_vars_chunks_offsets([varfinal.ind], fun_global.indsj, dimfinalchunk, dimfinalchunk_curr, varfinal.dim,
...
class GpuReduc1D_finalchunks(MapReduce, Gpu_link_compile):
    ...
    def get_code(self):
        ...
        varloader_global = Var_loader(fun_internal)        
        ...
        declare_assign_indices_j = (
            f"int *indices_j = offsets + {nvarsi};" if nvarsj_global > 0 else ""
        )
        declare_assign_indices_p = (
            f"int *indices_p = offsets + {nvarsi} + {nvarsj};" if nvarsp_global > 0 else ""
        )
                
        chunk_sub_routine = do_finalchunk_sub_ranges(dtype, fun_global, varfinal, dimfinalchunk,
                                              acc, i, j, jstart, start_y, chunk, end_x, end_y, nbatchdims, indices_j, arg, fout, yj, out)
        
        chunk_sub_routine_last = do_finalchunk_sub_ranges(dtype, fun_global, varfinal, dimlastfinalchunk,
                                                acc, i, j, jstart, start_y, lastchunk, end_x, end_y, nbatchdims, indices_j, arg, fout, yj, out)
        ...
        self.code = f"""
                        ...
                        extern "C" __global__ void GpuConv1DOnDevice_ranges(int nx, int ny, int nbatchdims,
                                                    int *shapes, int *offsets_d, int *lookup_d, int *slices_x,
                          ...
                          if (i < end_x) {{
                              ...
                              if (nbatchdims == 0) {{
                                  ...
                                  {load_vars(dimsx, indsi, xi, args, row_index=threadIdx_x, offsets=indices_i)} // Possibly, with offsets as we support broadcasting over batch dimensions
                              }}
                                  ...
                                  for (int jstart = start_y, tile = 0; jstart < end_y; jstart += blockDim.x, tile++) {{
                                      ...
                                      if (j < end_y) {{ // we load yj from device global memory only if j<end_y
                                          if (nbatchdims == 0) {{
                                              ...
                                              {load_vars(dimsy, indsj, yjloc, args, row_index=j-starty, offsets=indices_j)} // Possibly, with offsets as we support broadcasting over batch dimensions
                                          }}
                                      }}
                                      __syncthreads();
