...
def do_chunk_sub_ranges(dtype, red_formula, fun_chunked_curr, dimchunk_curr,
                                ...
                                acc, tile, i, j, jstart, start_y, chunk, end_x, end_y, nbatchdims,
                                indices_i, indices_j, arg, fout, xi, yj, param_loc):
    ...
    load_chunks_routine_i_batches = load_vars_chunks_offsets(indsi_chunked, indsi_global, dimchunk, dimchunk_curr, chk.dim_org, 
                                                xiloc, arg, chunk, indices_i, row_index=threadIdx_x)
    
    load_chunks_routine_j = load_vars_chunks(indsj_chunked, dimchunk, dimchunk_curr, chk.dim_org, 
                                                yjloc, arg, chunk, row_index=j)
    
    load_chunks_routine_j_batches = load_vars_chunks_offsets(indsj_chunked, indsj_global, dimchunk, dimchunk_curr, chk.dim_org, 
...
class GpuReduc1D_ranges_chunks(MapReduce, Gpu_link_compile):
    ...
    def get_code(self):
        ...
        chunk_sub_routine = do_chunk_sub_ranges( dtype, red_formula, chk.fun_chunked, dimchunk,
                                chk.dimsx, chk.dimsy,
                                chk.indsi, chk.indsj,
                                chk.indsi_chunked, chk.indsj_chunked,
                                acc, tile, i, j, jstart, starty, chunk, end_x, end_y, nbatchdims,
                                indices_i, indices_j, arg, fout_chunk, xi, yj, param_loc)
        
        last_chunk = c_variable("int", f"{chk.nchunks-1}")
        chunk_sub_routine_last = do_chunk_sub_ranges( dtype, red_formula, chk.fun_lastchunked, chk.dimlastchunk,
