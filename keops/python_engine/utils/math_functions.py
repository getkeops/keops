...
keops_sinxdivx = math_function(cpu_code = lambda x : f"({x} ? {keops_sin(x)}/{x} : 1.0f)") 
...
keops_xlogx = math_function(cpu_code = lambda x : f"({x} ? {x} * {keops_log(x)} : 0.0f)")
...
keops_sincos = math_function(cpu_code = lambda x, s, c : f"*{s}=sin(x); *{c}=cos(x);") 
