...
//void check_ranges(size_t nargs, array_t* obj_ptr) {
  ...
  int nbatchdims, *shapes;
  
  #if C_CONTIGUOUS
    int MN_pos = nbatchdims;
    int D_pos = nbatchdims + 1;
    auto get_size_batch = [](auto obj_ptr, int nbatch, int b) {
       return get_size(*obj_ptr, b);
    };
  #else
    int MN_pos = nbatchdims + 1;
    int D_pos = nbatchdims;
    auto get_size_batch = [](auto obj_ptr, int nbatch, int b) {
