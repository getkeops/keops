...
array_t launch_keops(int tag1D2D,
  ...
  switch (decision) {
    case 0: {
      auto result_array = allocate_result_array< at::Tensor >(shape_out, nbatchdims);
    ...
    default: {
      throw std::runtime_error("[KeOps] Inconsistant values for tagHostDevice, tagCpuGpu, tagRanges, tag1D2D...");
    }
  }
