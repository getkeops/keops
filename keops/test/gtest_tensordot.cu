...
TEST(tensordot, zeros){
  ...
  EvalRed<GpuConv1D_FromHost>(Sum_Reduction(f,0),1, 1, out_keops, FA, FB);

  auto f_legacy = TensorProd(x,y);
  __TYPE__ out_legacy[16];
  EvalRed<GpuConv1D_FromHost>(Sum_Reduction(f_legacy,0),1, 1, out_legacy, FA, FB);
...
TEST(tensordot, one){
  ...
  EvalRed<GpuConv1D_FromHost>(Sum_f,1, 1, out_keops, FA, FB);
...
TEST(tensordot, two){
  ...
  __TYPE__ s2d = 0;
  for(int i=0; i<2; i++) {
    // std::cout << std::setprecision(20) << out_keops[i] << "      " << std::setprecision(20) << out_loop[i] << std::endl;
    s2d += abs(out_keops[i] - out_loop[i]);
  }
  EXPECT_LE(s2d,5e-6);
}
...
TEST(tensordot, three){
  ...
  EvalRed<GpuConv1D_FromHost>(Sum_f,1, 1, out_keops, FAA, FBB);
...
TEST(tensordot, four){
  ...
  __TYPE__ out_keops;
  EvalRed<GpuConv1D_FromHost>(Sum_f,1, 1, &out_keops, FAA, FAA);
...
TEST(tensordot, nine){
  ...
  EvalRed<GpuConv1D_FromHost>(Sum_Reduction(f,0),1, 1, out_keops, FA, FB, XI);

  auto f_legacy = Grad(TensorProd(x,y),x,xi);
  __TYPE__ out_legacy[4];
  EvalRed<GpuConv1D_FromHost>(Sum_Reduction(f_legacy,0),1, 1, out_legacy, FA, FB, XI);
...
TEST(tensordot, seventeen){
  ...
  __TYPE__ XI[3] = {6, 4, 2};
  __TYPE__ out_keops[8];
  EvalRed<GpuConv1D_FromHost>(Sum_f,1, 1, out_keops, FAA, FBB, XI);
