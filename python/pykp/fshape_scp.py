...
if __name__ == '__main__':
    ...
    if True:
        ...
        x     =(np.linspace(1,2,sizeX)[:,np.newaxis] * np.ones((1,dimPoint))).astype('float32')
        alpha =(np.linspace(1,2,sizeX)[:,np.newaxis] * np.ones((1,dimPoint))).astype('float32')
        f     =(np.linspace(1,2,sizeX)[:,np.newaxis] ).astype('float32')
        y     =(np.linspace(1,2,sizeY)[:,np.newaxis] * np.ones((1,dimVect)) ).astype('float32')
        beta  =(np.linspace(1,2,sizeY)[:,np.newaxis] * np.ones((1,dimVect)) ).astype('float32')
        g     =(np.linspace(1,2,sizeY)[:,np.newaxis] ).astype('float32')
    
    # Call cuda kernel
    gamma = np.zeros(sizeX).astype('float32')
    cuda_shape_scp(x, y, f, g, alpha, beta, gamma, sigma_geom, sigma_sig, sigma_var,"cauchy","gaussian", "binet")
    ...
    gamma_py = np.sum( (areaa[:,np.newaxis] * areab[np.newaxis,:]) *  cauchy(squdistance_matrix(x,y),sigma_geom) *  gaussian(squdistance_matrix(f,g),sigma_sig) * binet(nalpha @ nbeta.T), axis=1 )
