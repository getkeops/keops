...
## Reticulate config (use only if issue with Python)

```{r dev-reticulate}
# choose a python version
reticulate::py_require(packages="pykeops", python_version="3.12")
# check Python config
...
### Metadata (do not run if no modification)

:warning: Code chunk in this section should be run only if something is modified. :warning:
...
```{r dev-test}
# run the unit tests
Sys.setenv(TEST_GPU = 1)
Sys.setenv(RUN_LONG_TEST = 1)
tic("Unit tests run")
...
# Run examples
tic("Example run")
devtools::run_examples(pkg_dir, fresh = TRUE, run_donttest = TRUE)
toc()
```

```{r dev-check}
# fully check the package
Sys.setenv(TEST_GPU = 0)
Sys.setenv(RUN_LONG_TEST = 0)
tic("Package check")
