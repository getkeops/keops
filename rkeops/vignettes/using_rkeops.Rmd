...
**Important**: due to a code refactoring, `rkeops` is not available on CRAN at the moment. Please refer to the next section for installing the development version from Github.
...
The compilation of new operators produces shared library (shared object `.so`) files stored in a dedicated cache directory, to be reused and avoid recompilation of already defined operators.

You can check where your compiled operators are stored by running `get_rkeops_cache_dir()`. To clean RKeOps install and remove all shared library files, you can run `clean_rkeops()`.

You can modify this cache dir by using `get_rkeops_cache_dir(path)` where `path` indicates the path to the directory that you want to use.
