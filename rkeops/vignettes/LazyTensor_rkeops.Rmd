...
LazyTensors objects are wrappers around R matrices or vectors that are used to create symbolic formulas for the KeOps reduction operations. A typical use case is the following: 
...
Here, `x` is a `LazyTensor`. The operations `sum()`, `min()`, `argmin()`, `max()` and `argmax()` can be called with `NA` index (default), in which case no reduction is done and the result is a `LazyTensor`. Otherwise, the result is not a `LazyTensor`. 
...
| `sumsoftmaxweight(x, "j", w)` $\qquad$  `sumsoftmaxweight_reduction(x, "j", w)` | "Sum of weighted Soft-Max" reduction indexed by `j` of the elements of `x` | $\left(\sum_j\exp(x_{ij})w_{ij}\right)/\left(\sum_j\exp(x_{ij})\right)$         |
...
* a (big) note on `ComplexLazyTensor`, and maybe on how they are encodes (real column/imaginary column) ?
* add "or `ComplexLazyTensor`" each (almost) time we write "Here, `x` is a `LazyTensor`..." ?
