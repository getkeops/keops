...
__global__ void VarFunDXGaussGpuConvOnDevice(TYPE ooSigmax2, TYPE ooSigmaf2, TYPE ooSigmaXi2, 
				     TYPE *x, TYPE *y, 
				     TYPE *f, TYPE *g, 
				     ...
				     int nx, int ny)
{
 ...
 // distance between points and signals
		...
		TYPE s = sqrt(norm2Xix * norm2Xiy) * dKernel_geom1(dist2_geom,ooSigmax2) * Kernel_sig1(dist2_sig,ooSigmaf2) * Kernel_var1(angle_normals,ooSigmaXi2);
...
extern "C" int VarFunDXGaussGpuEvalConv_float(float ooSigmax2,float ooSigmaf2, float ooSigmaXi2,
                                   float* x_h, float* y_h,
                                   float* f_h, float* g_h,
                                   ...
                                   int dimPoint, int dimSig, int dimVect, int nx, int ny)
{
    ...
    if(dimPoint==1 && dimSig==1 && dimVect==1)
        VarFunDXGaussGpuConvOnDevice<float,1,1,1><<<gridSize,blockSize,blockSize.x*(dimVect+dimSig+dimPoint)*sizeof(float)>>>
        (ooSigmax2,ooSigmaf2,ooSigmaXi2, x_d, y_d, f_d, g_d, alpha_d, beta_d, gamma_d, nx, ny);
    else if(dimPoint==3 && dimSig==1 && dimVect==1)
        VarFunDXGaussGpuConvOnDevice<float,3,1,1><<<gridSize,blockSize,blockSize.x*(dimVect+dimSig+dimPoint)*sizeof(float)>>>
        (ooSigmax2,ooSigmaf2,ooSigmaXi2, x_d, y_d, f_d, g_d, alpha_d, beta_d, gamma_d, nx, ny);
    else if(dimPoint==2 && dimSig==1 && dimVect==1)
        VarFunDXGaussGpuConvOnDevice<float,2,1,1><<<gridSize,blockSize,blockSize.x*(dimVect+dimSig+dimPoint)*sizeof(float)>>>
        (ooSigmax2,ooSigmaf2, ooSigmaXi2, x_d, y_d, f_d, g_d, alpha_d, beta_d, gamma_d, nx, ny);
    else if(dimPoint==2 && dimSig==1 && dimVect==2)
        VarFunDXGaussGpuConvOnDevice<float,2,1,2><<<gridSize,blockSize,blockSize.x*(dimVect+dimSig+dimPoint)*sizeof(float)>>>
        (ooSigmax2,ooSigmaf2, ooSigmaXi2, x_d, y_d, f_d, g_d, alpha_d, beta_d, gamma_d, nx, ny);
    else if(dimPoint==3 && dimSig==1 && dimVect==3)
        VarFunDXGaussGpuConvOnDevice<float,3,1,3><<<gridSize,blockSize,blockSize.x*(dimVect+dimSig+dimPoint)*sizeof(float)>>>
        (ooSigmax2,ooSigmaf2, ooSigmaXi2, x_d, y_d, f_d, g_d, alpha_d, beta_d, gamma_d, nx, ny);
    else
    {
...
#if UseCudaOnDoubles  
extern "C" int VarFunDXGaussGpuEvalConv_double(double ooSigmax2,double ooSigmaf2, double ooSigmaXi2,
                                   ...
                                   int dimPoint, int dimSig, int dimVect, int nx, int ny)
{
    ...
    if(dimPoint==1 && dimSig==1 && dimVect==1)
        VarFunDXGaussGpuConvOnDevice<double,1,1,1><<<gridSize,blockSize,blockSize.x*(dimVect+dimSig+dimPoint)*sizeof(double)>>>
        (ooSigmax2,ooSigmaf2,ooSigmaXi2, x_d, y_d, f_d, g_d, alpha_d, beta_d, gamma_d, nx, ny);
    else if(dimPoint==3 && dimSig==1 && dimVect==1)
        VarFunDXGaussGpuConvOnDevice<double,3,1,1><<<gridSize,blockSize,blockSize.x*(dimVect+dimSig+dimPoint)*sizeof(double)>>>
        (ooSigmax2,ooSigmaf2,ooSigmaXi2, x_d, y_d, f_d, g_d, alpha_d, beta_d, gamma_d, nx, ny);
    else if(dimPoint==2 && dimSig==1 && dimVect==1)
        VarFunDXGaussGpuConvOnDevice<double,2,1,1><<<gridSize,blockSize,blockSize.x*(dimVect+dimSig+dimPoint)*sizeof(double)>>>
        (ooSigmax2,ooSigmaf2, ooSigmaXi2, x_d, y_d, f_d, g_d, alpha_d, beta_d, gamma_d, nx, ny);
    else if(dimPoint==2 && dimSig==1 && dimVect==2)
        VarFunDXGaussGpuConvOnDevice<double,2,1,2><<<gridSize,blockSize,blockSize.x*(dimVect+dimSig+dimPoint)*sizeof(double)>>>
        (ooSigmax2,ooSigmaf2, ooSigmaXi2, x_d, y_d, f_d, g_d, alpha_d, beta_d, gamma_d, nx, ny);
    else if(dimPoint==3 && dimSig==1 && dimVect==3)
        VarFunDXGaussGpuConvOnDevice<double,3,1,3><<<gridSize,blockSize,blockSize.x*(dimVect+dimSig+dimPoint)*sizeof(double)>>>
        (ooSigmax2,ooSigmaf2, ooSigmaXi2, x_d, y_d, f_d, g_d, alpha_d, beta_d, gamma_d, nx, ny);
    else
    {
        printf("error: dimensions of Gauss kernel not implemented in cuda");
...
//////////////////////////////////////////////////////////////////
 ...
 { 
  ...
  //------ the sixth input argument: beta---------------//
...
#if UseCudaOnDoubles   
   VarFunDXGaussGpuEvalConv_double(oosigmax2,oosigmaf2,oosigmaXi2,x,y,f,g,alpha,beta,gamma,dimpoint,dimsig,dimvect,nx,ny);
#else
   // convert to float
   ...
   VarFunDXGaussGpuEvalConv_float(oosigmax2,oosigmaf2,oosigmaXi2,x_f,y_f,f_f,g_f,alpha_f,beta_f,gamma_f,dimpoint,dimsig,dimvect,nx,ny);
