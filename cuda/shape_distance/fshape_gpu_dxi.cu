				     ...
				     int nx, int ny)
{
    ...
    for(int jstart = 0, tile = 0; jstart < ny; jstart += blockDim.x, tile++)
    {
	    ...
	    for(int jrel = 0; jrel < blockDim.x && jrel<ny-jstart; jrel++, yj+=inc, gj +=inc, betaj+=inc)
	    {
		    ...
		    TYPE Kv =  Kernel_var1(angle_normals,ooSigmaXi2);
		    TYPE dKv =  dKernel_var1(angle_normals,ooSigmaXi2);
		    for (int k = 0; k < DIMVECT; k++)
		    {
			    gammai[k] += KgKs * ( (sqrt(norm2Xiy / norm2Xix) * (Kv - dKv * angle_normals) *  alphai[k] ) + dKv  * betaj[k]);
		    }
	    }
        }
        __syncthreads();
...
extern "C" int VarFunGaussGpuEvalConv_float(float ooSigmax2,float ooSigmaf2, float ooSigmaXi2,
                                   float* x_h, float* y_h,
                                   float* f_h, float* g_h,
...
#if UseCudaOnDoubles  
extern "C" int VarFunGaussGpuEvalConv_double(double ooSigmax2,double ooSigmaf2, double ooSigmaXi2,
...
//////////////////////////////////////////////////////////////////
 ...
 { 
  ...
  //------ the sixth input argument: beta---------------//
...
#if UseCudaOnDoubles   
   VarFunGaussGpuEvalConv_double(oosigmax2,oosigmaf2,oosigmaXi2,x,y,f,g,alpha,beta,gamma,dimpoint,dimsig,dimvect,nx,ny);
#else
   // convert to float
   ...
   VarFunGaussGpuEvalConv_float(oosigmax2,oosigmaf2,oosigmaXi2,x_f,y_f,f_f,g_f,alpha_f,beta_f,gamma_f,dimpoint,dimsig,dimvect,nx,ny);
