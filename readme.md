...
A small plugin to pytorch, numpy, Matlab or R that enables
these frameworks to compute efficiently **Kernel dot products**
...
As of 2018, a considerable effort has been made towards the main
operations involved in the study of Convolutional Neural Networks:
linear algebra routines and *grid* convolutions.

However, as researchers focused on statistical shape analysis,
we got confronted with a **memory bottleneck** when trying
to compute Gaussian convolutions on a non regular grid in $`\mathbb R^3`$
...
we need to compute efficiently the array $`(\alpha_i)_{i=1}^N \in  \mathbb R^{N \times D}`$ given by
...
A standard way of doing so in array-centric frameworks


For large datasets or point clouds (say, if M,N >= 100,000),
this method is intractable:
