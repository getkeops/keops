...
y = torch.rand(1, N, 2 * 2, requires_grad=True)
...
print("Check tensordot computation. All good ?", torch.allclose(sum_f_keops,sum_f_torch))


# checking gradients
u = torch.rand(M, 8)
grad_keops = torch.autograd.grad(sum_f_keops, x, u, retain_graph=True)[0]
grad_torch = torch.autograd.grad(sum_f_torch, x, u, retain_graph=True)[0]
print("Check gradient wrt x. All good ?", torch.allclose(grad_keops,grad_torch))


grad_keops = torch.autograd.grad(sum_f_keops, y, u)[0]
grad_torch = torch.autograd.grad(sum_f_torch, y, u)[0]

print(grad_keops)
print(grad_torch)
print("Check gradient wrt y. All good ?", torch.allclose(grad_keops,grad_torch))
