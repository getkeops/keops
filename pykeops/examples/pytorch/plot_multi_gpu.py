...
print('time for multi-Gpu computation (block parallelized over reduction index):{:.2f} s'.format(elapsed))
...
for gpuid in range(ngpus):
    out_multi_2[gpuid*subsize_y:(gpuid+1)*subsize_y,:] = futures[gpuid].result().cpu()
elapsed = time.time() - start
print('time for multi-Gpu computation (block parallelized over output index):{:.2f} s'.format(elapsed))
