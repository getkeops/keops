...
for bench_method in (bench_time,benchmark):
    ...
    print(bench_method(ker_gauss_raw,(out,x,y,b),n_repeat=n_repeat))
    if N<20000:
        out_ref = cp.zeros(N, dtype=cp.float32)
        print(bench_method(ker_gauss_cupy,(out_ref,x,y,b),n_repeat=n_repeat))
        print("relative error : ", cp.linalg.norm(out-out_ref)/cp.linalg.norm(out_ref))
        print(bench_method(ker_gauss_torch,(out_ref,x,y,b),n_repeat=n_repeat))