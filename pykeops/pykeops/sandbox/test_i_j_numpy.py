...
fun1 = Genred("Exp(i-j*SqDist(x,y))*b",[f"x=Vi({D})", f"y=Vj({D})", f"b=Vj({DV})"], axis=axis)
fun2 = Genred("Exp(i-j*SqDist(x,y))*b",[f"x=Vi({D})", f"y=Vj({D})", f"b=Vj({DV})", "i=Vi(1)", "j=Vj(1)"], axis=axis)
res1 = fun1(x, y, b)
res2 = fun2(x, y, b, i, j)
