...
for backend in backends:
    ...
    start = time.time()
    for i in range(ntry):
        fun(x, y, b, backend).squeeze()
    end = time.time()
    print("average time for " + backend + ":", (end - start) / ntry)
...
if test_grad:
    ...
    for k, backend in enumerate(backends):
        ...
        for i in range(ntry):
            torch.autograd.grad(
                (out[k] ** 2).sum(), [x], create_graph=True, retain_graph=True
            )[0]
        end = time.time()
        print("average time for " + backend + " (grad):", (end - start) / ntry)
        out_g.append(
...
if test_grad2:
    ...
    for k, backend in enumerate(backends):
        start = time.time()
        for i in range(ntry):
            torch.autograd.grad((out_g[k] ** 2).sum(), [x], retain_graph=True)[0]
        end = time.time()
        print("average time for " + backend + " (grad2):", (end - start) / ntry)
