...
def preprocess_bf162(args, aliases, axis, ranges, nx, ny):
    N = ny if axis == 1 else nx

    if ranges:
        # When using ranges, we need to adapt the ranges to the special copy trick
        if axis == 1:
            ranges = ranges2bf162(ranges[0:3], ny) + ranges[3:6]
        else:
            ranges = ranges[0:3] + ranges2bf162(ranges[3:6], nx)
...
def postprocess_bf162(out, tag_dummy, reduction_op, N):
    out = bf1622bf16(out)
    if tag_dummy:
        out = out[..., :-1, :]
