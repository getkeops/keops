...
def test_kron_lazytensor_torch():
    ...
    assert torch.allclose(gamma_keops_lazytensor_torch, torch.from_numpy(gamma_py), atol=1e-6)
...
for i in range(M):
    for j in range(N):
        gamma_py2[i, j, :] = np.kron(x0[i, :].reshape(Ddx, Ddx2),
                                     y0[j, :].reshape(Ddx, Ddx)).flatten()
...
for i in range(M):
    for j in range(N):
        gamma_py3[i, j, :] = np.kron(x2[i, :].reshape(Ddx, Ddx2, Ddx), y2[j, :].reshape(Ddx, Ddx, Ddx2)).flatten()
...
for i in range(M):
    for j in range(N):
        gamma_py4[i, j, :] = np.einsum('ijk, lmn -> iljmkn',
                                       x2[i, :].reshape(Ddx, Ddx2, Ddx),
                                       y2[j, :].reshape(Ddx, Ddx, Ddx2)).flatten()
