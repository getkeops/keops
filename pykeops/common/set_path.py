...
def set_bin_folder(bf_user=None, append_to_python_path=True):
    ...
    if append_to_python_path:
        ...
        if any(any(i in s for i in ["libKeOps", "fshape_scp", "radial_kernel"]) for s in
               sys.modules.keys()):  # check that no pykeops modules were imported.
            warnings.warn("[pyKeOps]: set_bin_folder() has been invoked while some pykeops modules have already "
...
def create_name(formula, aliases, dtype, lang, optional_flags):
    ...
    dll_name = "libKeOps" + lang + sha256((pykeops.config.bin_folder + dll_name).encode("utf-8")).hexdigest()[:10]
    return dll_name
...
def clean_pykeops(path="", lang=""):
    ...
    for f in os.scandir(path):
        if f.is_dir(follow_symlinks=False) and \
                (f.name.count("build-libKeOps" + lang) or f.name.count("build-fshape_scp") or f.name.count(
                    "build-radial_kernel")):
