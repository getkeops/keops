...
def compile_specific_routine(dllname, cuda_type):
    ...
    print("\n")
    subprocess.run(["cmake", script_folder, "-DPYTHON_LIB=TRUE", "-Dshared_obj_name="+dllname, "-D__TYPE__="+cuda_type], cwd=build_folder, check=True)
    subprocess.run(["make", dllname], cwd=build_folder,check=True)
    print("Done. ", end='')
