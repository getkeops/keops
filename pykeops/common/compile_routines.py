...
def check_or_prebuild(dtype, lang, include_dirs):
    ...
    if not os.path.exists(build_folder + os.path.sep + 'CMakeCache.txt'):
        print('[pyKeOps] Initializing build folder for dtype=' + str(dtype) +
              ' and lang=' + lang + ' in ' + os.path.realpath(pykeops.config.bin_folder) +
              ' ... ', end='', flush=True)
        run_and_display(command_line + ["-DcommandLine=" + " ".join(command_line)],
                        build_folder,
                        msg="CMAKE")
...
def build_keops_formula_object_file(build_folder, dtype, formula, alias_string, optional_flags):
    ...
    run_and_display(["cmake", "--build", ".", "--target", pykeops.config.shared_obj_name, "--", "VERBOSE=1"],
                    build_folder, msg="MAKE")
...
def compile_specific_conv_routine(dllname, dtype, build_folder):
    print('Compiling ' + dllname + ' using ' + dtype + ' in ' + os.path.realpath(build_folder + os.path.sep + '..') +
          '... ', end='', flush=True)
