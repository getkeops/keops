...
if __name__ == '__main__':
    ...
    cuda_shape_scp(x, y, f, g, alpha, beta, gamma, sigma_geom, sigma_sig, sigma_var,kgeom,ksig,ksphere)
    ...
    gamma_py = np.sum( (areaa[:,np.newaxis] * areab[np.newaxis,:]) *  np_kernel(x,y,sigma_geom,kgeom) *  np_kernel(f,g,sigma_sig,ksig) * np_kernel_sphere(nalpha,nbeta,sigma_var,ksphere), axis=1 )
